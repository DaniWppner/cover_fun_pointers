...
def filter_populated_pointers(data: list[dict]) -> list[dict]:

    return [entry for entry in data if len(entry["function_pointers"]) > 0]
...
if __name__ == "__main__":
    ...
    print(f"Info: Wrote json ({len(filtered_json)} entries, filtered out of {len(extracted_json)})")
