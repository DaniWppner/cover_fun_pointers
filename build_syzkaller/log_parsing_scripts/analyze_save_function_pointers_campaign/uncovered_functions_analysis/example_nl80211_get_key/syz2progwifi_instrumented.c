...
FILE *global_fout_pcs = NULL;
FILE *global_fout_fps = NULL;
...
static void process_kcov_buffer(unsigned long *cover) {
    ...
    if (!global_fout_pcs || !global_fout_fps) {
        fprintf(stderr, "[-] Global coverage files not open!\n");
        return;
    }
    ...
    // We flush instead of close so that if loop() executes multiple times, it doesn't crash on subsequent calls.
    fflush(global_fout_pcs);
    fflush(global_fout_fps);    
    printf("[+] PC coverage data written successfully.\n");
    printf("[+] Function Pointer coverage data written successfully.\n");
}
...
static void process_kcov_result(int kcov_fd, unsigned long *cover) {
...
//  returns sock_nl_generic
	res = syscall(__NR_socket, /*domain=*/0x10ul, /*type=*/3ul, /*proto=*/0x10);
        ...
        printf("[+] socket(AF_NETLINK, SOCK_RAW, NETLINK_GENERIC) returned: %ld\n", res);
        if (res != -1)
		r[0] = res;
...
res = syz_genetlink_get_family_id(/*name=*/0x200000000040, /*fd=*/r[0]);
        ...
        printf("[+] syz_genetlink_get_family_id(\"nl80211\") returned: %ld\n", res);
        if (res != -1)
		r[1] = res;
...
memcpy((void*)0x200000000080, "wlan0\000\000\000\000\000\000\000\000\000\000\000", 16);
	res = syscall(__NR_ioctl, /*fd=*/r[0], /*cmd=*/0x8933, /*arg=*/0x200000000080ul);
        ...
        printf("[+] ioctl(SIOCGIFINDEX, \"wlan0\") returned: %ld (errno=%d)\n", res, res == -1 ? errno : 0);
...
*(uint32_t*)0x2000000000f0 = 0x8014;
	unsigned long *cover;
        ...
        printf("[*] Sending NL80211_CMD_GET_KEY sendmsg...\n");
...
int main(void)
{
	...
	(void)reason;
        ...
        global_fout_pcs = fopen("syz2progwifi_instrumented_PCs.cov", "w");
        global_fout_fps = fopen("syz2progwifi_instrumented_FPs.cov", "w");
        if (!global_fout_pcs || !global_fout_fps) {
            perror("[-] Failed to open global coverage files");
            return 1;
        }
