...
def find_first_line_after_timestamp(lines: list[str], cutoff: datetime) -> int:
    ...
    while low < high:
        ...
        if mid_time <= cutoff:
            ...
            high = mid
        print(f"DEBUG: mid:{mid}, low:{low}, high:{high}")
...
def find_cutoff_before_status_block(lines: list[str], cutoff: datetime) -> int:
    ...
    print(f"DEBUG: Chosen idx{cutoff_idx}")
...
def parse_raw_triage_logs(log_lines: list[str]) -> Iterable[dict]:
    ...
    indentify_fpointer_cover_re = re.compile(
    r"\[(prog-[^\]]+)\] stored function pointers in call (#\d+ \[[^\]]+\])(?:: (\[.*\]))?"
    )
    triage_line_re, triage_re_map = create_triage_master_regex()

    for line_number, line in tcontrib.tenumerate(log_lines):
        adhoc_match = indentify_fpointer_cover_re.search(line)
        if adhoc_match:
            entry = adhoc_fpointer_empty_syzkaller_parse(adhoc_match)
            yield entry
...
def adhoc_fpointer_empty_syzkaller_parse(adhoc_match):
    prog_id = adhoc_match.group(1)
    call_id = adhoc_match.group(2)
    fpointer_cover = adhoc_match.group(3)
    entry = {RESULT_KEYS.PROGID: prog_id,
                     RESULT_KEYS.CALL_NAME: call_id}
    if fpointer_cover is not None:
        fpointer_payload = fpointer_cover
    else:
        fpointer_payload = "[]"
    entry[RESULT_KEYS.NEW_FPOINTERS_PAYLOAD] = fpointer_payload
    return entry
...
if __name__ == "__main__":
    ...
    if args.cutoff_time is not None:
        ...
        print(f"DEBUG: Chosen final idx: {cutoff_index}")
