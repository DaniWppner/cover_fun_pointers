...
def unify_per_prog(json_lines_file: Path) -> dict:
    ...
    with open(json_lines_file) as f:
        for line in f:
            ...
            #hack: in this version the fpointer coverage comes first
            if RESULT_KEYS.NEW_FPOINTERS_PAYLOAD in log_entry:
            ...
            # hack since fpointer coverage comes first, now "no new" refers
            # to "have we set the original prog for this entry yet?"
            no_new: bool = data_key in result_dict and RESULT_KEYS.ORIGINAL_PROG in result_dict[data_key]
...
def check_easy_stats(prog2log: dict[str, dict]) -> None:
    #hack: for some reason this version can not have count
    n_duplicate_progs = count_cond(prog2log, lambda e: RESULT_KEYS.COUNT in e and e[RESULT_KEYS.COUNT] > 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cut off analysis after this number of unique triage. "
        ),
    )
    ...
    if args.n_triage is not None:
        print(f"[INFO] Cutting out triage. Len before: {len(out_json)}", end='.', file=sys.stderr)
        out_json = dict(list(out_json.items())[0:args.n_triage])
        print(f"Len after: {len(out_json)}",file=sys.stderr)
